...
private:
    ...
    //Modify find method so that a search of the tree also stores the depth of the last node visited.
    //pass an integer by reference into both methods and modify it inside the methods.
    int find(const Comparable &c, BinaryNode* n, int &depth) const {
        ...
        if (c < n->value) {
            ...
            return find(c, n->leftChild, currDepth);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to node's right child.
            int currDepth = depth + 1;
            return find(c, n->rightChild, currDepth);
        }
